...
/**
 * Points («очки», never «монеты») of a session (SPEC §15.3): while `score ≥ 0.5` a
 * portion of `1 + round(2·ring)` is awarded every `0.6 − 0.48·ring` seconds — the more
 * of the sound sits in the cupola, the faster they come.
 */
class PointsCounter(
    ...
    fun update(timeSec: Double, score: Double, ring: Double): Int {
        if (score < minScore) {
